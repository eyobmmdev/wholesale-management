...
import { useCreateSaleItem } from '../../hooks/useSales.js';
import { saleService } from '../../services/saleService.js';
import { showToast } from '../../utils/toast.js';
import { handleBackendErrors } from '../../utils/errorHandler.js';

export default function SaleItemAddForm({ saleId, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    const toastId = showToast.loading('Adding sale item...');
    ...
    createMutation.mutate(payload, {
      onSuccess: () => {
        showToast.success('Success', 'Sale item added successfully');
        showToast.dismiss(toastId);
        onClose();
      },
      onError: (err) => {
        showToast.dismiss(toastId);
        handleBackendErrors(err, setErrors, 'Failed to add sale item');
      }
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="common-form" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }} noValidate>
      ...
      <div className="form-actions" style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '8px' }}>
        <Button type="button" variant="outline" onClick={onClose} disabled={createMutation.isLoading}>
          Cancel
        </Button>
        <Button type="submit" variant="primary" disabled={createMutation.isLoading} isLoading={createMutation.isLoading}>
          Add Item
        </Button>
      </div>
    </form>
